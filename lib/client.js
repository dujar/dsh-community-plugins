  ...
  factory: (require) => {
    ...
    function ForksDialog(props) {
      ...
      return createElement('div', {
        ...
        createElement('div', {
          ...
          createElement(Notice, { notice: props.notice }),
    ...
    function LocalCard(props) {
      ...
      return createElement('div', {
        ...
        // The shipped README, when there is one, is the showcase: rendered
        // (safely — a minimal markdown subset) in a clamped, scrollable box.
        item.readme ? createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
          createElement('span', {
          ...
          item.readmeTruncated ? createElement('p', { style: { margin: 0, fontSize: 11, color: 'var(--dsw-alias-label-tertiary)' } }, t('readmeTruncated')) : null,
        ) : null,
    ...
    function CommunityPluginsTab(props) {
      ...
      function content() {
        ...
        // Installed plugins the community catalog does not know about render as
        // local cards — no GitHub data to link, uninstall by package name.
        if (filter === 'local') {
          return createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 10 } },
            items.map(function (item) {
              return createElement(LocalCard, {
                key: item.name || item.full_name, t: t, item: item,
                busy: busy === (item.name || item.full_name),
      ...
      return createElement('div', {
        ...
        // notice banner
        createElement(Notice, { notice: notice }),
        // status / count row
