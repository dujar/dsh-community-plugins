...
async function findReadme(dir) {
  ...
  const hit = entries.find((entry) => /^readme(?:[.](?:md|markdown|txt))?$/i.test(entry))
  if (!hit) return null
  try {
    return await readFile(join(dir, hit), 'utf8')
  } catch {
    return null
  }
}
...
export async function enrichLocal(profile, item) {
  ...
  for (const candidate of localManifestCandidates(profile, item.name, item.spec)) {
    try {
      ...
      readme = await findReadme(dirname(candidate))
      break
  ...
  const author = typeof manifest.author === 'string'
    ? manifest.author
    : (manifest.author && typeof manifest.author.name === 'string' ? manifest.author.name : null)
  const readmeTruncated = readme !== null && readme.length > README_LIMIT
  return {
    ...item,
    version: typeof manifest.version === 'string' ? manifest.version : null,
    description: typeof manifest.description === 'string' ? manifest.description : null,
    author: author || null,
    readme: readme === null ? null : readme.slice(0, README_LIMIT),
    readmeTruncated,
  }
}
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (filter === 'local') {
          ...
          result = { ...local, items: await Promise.all(local.items.map((item) => enrichLocal(profile, item))) }
        } else {
