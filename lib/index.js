...
/** Hard ceiling for one install/uninstall invocation. */
...
function runDshPlugin(profile, args) {
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      finish({ ok: code === 0, code: code ?? null, output: (stdout + stderr).slice(-OUTPUT_TAIL) })
    })
  })
}
